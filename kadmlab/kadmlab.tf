...
resource "libvirt_cloudinit_disk" "kadmlablb" {
    ...
    user_data = templatefile("${path.module}/kadmlabnode_user_data.tftpl", { hostname = "kadmlablb${count.index + 1}", fqdn = "kadmlablb${count.index + 1}.kadmlab.prism.local", username = "jafager", password = "Ubuntu22.04", ssh_public_key = "ssh-rsa  jafager@prism" })
    network_config = templatefile("${path.module}/kadmlabnode_network_config.tftpl", { ip_addresses_1 = "[192.168.252.${20 + (count.index + 1)}/24]", gateway = "192.168.252.1", nameservers = "[192.168.252.1]", domains = "[kadmlab.prism.local]", ip_addresses_2 = "[192.168.251.${20 + (count.index + 1)}/24]", ip_addresses_3 = "[192.168.250.${20 + (count.index + 1)}/24]"})
}
...
resource "libvirt_domain" "kadmlablb" {
    ...
    depends_on = [libvirt_volume.kadmlablb, libvirt_network.kadmlab_management, libvirt_network.kadmlab_cluster, libvirt_network.kadmlab_service]
}
