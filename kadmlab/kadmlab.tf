...
resource "libvirt_cloudinit_disk" "kadmlabstor" {
    ...
    network_config = templatefile("${path.module}/kadmlabstor_network_config.tftpl", { ip_addresses_1 = "[192.168.252.${30 + (count.index + 1)}/24]", gateway = "192.168.252.1", nameservers = "[192.168.252.1]", domains = "[kadmlab.prism.local]", ip_addresses_2 = "[192.168.251.${30 + (count.index + 1)}/24]" })
}
...
resource "libvirt_domain" "kadmlabnode" {
    ...
    depends_on = [libvirt_volume.kadmlabnode, libvirt_network.kadmlab_management, libvirt_network.kadmlab_cluster, libvirt_network.kadmlab_metallb]
}
...
resource "libvirt_domain" "kadmlabstor" {
    ...
    depends_on = [libvirt_volume.kadmlabstor, libvirt_network.kadmlab_management, libvirt_network.kadmlab_cluster]
}
