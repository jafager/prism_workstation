terraform {
    required_providers {
        libvirt = {
            source = "dmacvicar/libvirt"
        }
    }
}

provider "libvirt" {
    uri = "qemu:///system"
}

variable "node_count" {
    type = number
    description = "The number of nodes to create."
    default = 6
}

variable "lb_count" {
    type = number
    description = "The number of load balancers to create."
    default = 2
}

resource "libvirt_network" "kadmlab_management" {
    name = "kadmlab_management"
    mode = "nat"
    domain = "kadmlab.prism.local"
    addresses = ["192.168.252.0/24"]
    dns {
        enabled = true
        forwarders {
            address = "8.8.8.8"
        }
        forwarders {
            address = "8.8.4.4"
        }
        hosts {
            hostname = "kadmlabnode1"
            ip = "192.168.251.11"
        }
        hosts {
            hostname = "kadmlabnode2"
            ip = "192.168.251.12"
        }
        hosts {
            hostname = "kadmlabnode3"
            ip = "192.168.251.13"
        }
        hosts {
            hostname = "kadmlabnode4"
            ip = "192.168.251.14"
        }
        hosts {
            hostname = "kadmlabnode5"
            ip = "192.168.251.15"
        }
        hosts {
            hostname = "kadmlabnode6"
            ip = "192.168.251.16"
        }
        hosts {
            hostname = "kadmlablb1"
            ip = "192.168.251.21"
        }
        hosts {
            hostname = "kadmlablb2"
            ip = "192.168.251.22"
        }
        hosts {
            hostname = "controlplane"
            ip = "192.168.251.100"
        }
    }
    dhcp {
        enabled = false
    }
}

resource "libvirt_network" "kadmlab_cluster" {
    name = "kadmlab_cluster"
    mode = "none"
    addresses = ["192.168.251.0/24"]
    dns {
        enabled = false
    }
    dhcp {
        enabled = false
    }
}

resource "libvirt_network" "kadmlab_service" {
    name = "kadmlab_service"
    mode = "none"
    addresses = ["192.168.250.0/24"]
    dns {
        enabled = false
    }
    dhcp {
        enabled = false
    }
}

resource "libvirt_network" "kadmlab_metallb" {
    name = "kadmlab_metallb"
    mode = "none"
    addresses = ["192.168.249.0/24"]
    dns {
        enabled = false
    }
    dhcp {
        enabled = false
    }
}

resource "libvirt_pool" "kadmlab" {
    name = "kadmlab"
    type = "dir"
    path = "/mnt/libvirt/kadmlab"
}

resource "libvirt_volume" "ubuntu2204" {
    name = "ubuntu2204.qcow2"
    pool = "kadmlab"
    format = "qcow2"
    source = "https://cloud-images.ubuntu.com/jammy/current/jammy-server-cloudimg-amd64.img"
    depends_on = [libvirt_pool.kadmlab]
}

resource "libvirt_volume" "kadmlabnode" {
    count = var.node_count
    name = "kadmlabnode${count.index + 1}.qcow2"
    pool = "kadmlab"
    format = "qcow2"
    size = 32 * 1024 * 1024 * 1024
    base_volume_name = "ubuntu2204.qcow2"
    depends_on = [libvirt_volume.ubuntu2204]
}

resource "libvirt_volume" "kadmlablb" {
    count = var.lb_count
    name = "kadmlablb${count.index + 1}.qcow2"
    pool = "kadmlab"
    format = "qcow2"
    size = 16 * 1024 * 1024 * 1024
    base_volume_name = "ubuntu2204.qcow2"
    depends_on = [libvirt_volume.ubuntu2204]
}

resource "libvirt_cloudinit_disk" "kadmlabnode" {
    count = var.node_count
    name = "kadmlabnode${count.index + 1}.iso"
    user_data = templatefile("${path.module}/kadmlabnode_user_data.tftpl", { hostname = "kadmlabnode${count.index + 1}", fqdn = "kadmlabnode${count.index + 1}.kadmlab.prism.local", username = "jafager", password = "Ubuntu22.04", ssh_public_key = "ssh-rsa  jafager@prism" })
    network_config = templatefile("${path.module}/kadmlabnode_network_config.tftpl", { ip_addresses_1 = "[192.168.252.${10 + (count.index + 1)}/24]", gateway = "192.168.252.1", nameservers = "[192.168.252.1]", domains = "[kadmlab.prism.local]", ip_addresses_2 = "[192.168.251.${10 + (count.index + 1)}/24]", ip_addresses_3 = "[192.168.250.${10 + (count.index + 1)}/24]", ip_addresses_4 = "[192.168.249.${20 + (count.index + 1)}/24]" })
}

resource "libvirt_cloudinit_disk" "kadmlablb" {
    count = var.lb_count
    name = "kadmlablb${count.index + 1}.iso"
    user_data = templatefile("${path.module}/kadmlablb_user_data.tftpl", { hostname = "kadmlablb${count.index + 1}", fqdn = "kadmlablb${count.index + 1}.kadmlab.prism.local", username = "jafager", password = "Ubuntu22.04", ssh_public_key = "ssh-rsa  jafager@prism" })
    network_config = templatefile("${path.module}/kadmlablb_network_config.tftpl", { ip_addresses_1 = "[192.168.252.${20 + (count.index + 1)}/24]", gateway = "192.168.252.1", nameservers = "[192.168.252.1]", domains = "[kadmlab.prism.local]", ip_addresses_2 = "[192.168.251.${20 + (count.index + 1)}/24]" })
}

resource "libvirt_domain" "kadmlabnode" {
    count = var.node_count
    name = "kadmlabnode${count.index + 1}"
    vcpu = "2"
    memory = "4096"
    disk {
        volume_id = libvirt_volume.kadmlabnode[count.index].id
    }
    network_interface {
        network_id = libvirt_network.kadmlab_management.id
    }
    network_interface {
        network_id = libvirt_network.kadmlab_cluster.id
    }
    network_interface {
        network_id = libvirt_network.kadmlab_service.id
    }
    network_interface {
        network_id = libvirt_network.kadmlab_metallb.id
    }
    cloudinit = libvirt_cloudinit_disk.kadmlabnode[count.index].id
    depends_on = [libvirt_volume.kadmlabnode, libvirt_network.kadmlab_management, libvirt_network.kadmlab_cluster, libvirt_network.kadmlab_service, libvirt_network.kadmlab_metallb]
}

resource "libvirt_domain" "kadmlablb" {
    count = var.lb_count
    name = "kadmlablb${count.index + 1}"
    vcpu = "1"
    memory = "4096"
    disk {
        volume_id = libvirt_volume.kadmlablb[count.index].id
    }
    network_interface {
        network_id = libvirt_network.kadmlab_management.id
    }
    network_interface {
        network_id = libvirt_network.kadmlab_cluster.id
    }
    cloudinit = libvirt_cloudinit_disk.kadmlablb[count.index].id
    depends_on = [libvirt_volume.kadmlablb, libvirt_network.kadmlab_management, libvirt_network.kadmlab_cluster]
}
